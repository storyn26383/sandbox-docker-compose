...
- **預設 mount `./.data/coding` 落 `/home/sandbox/coding`** 做 workspace（host IDE edit、容器內 SSH/`make shell` 入去 `~/coding` 即見）。`make init` 會預先 `mkdir`，避免 Docker daemon 以 root 創建令 sandbox 寫唔到。要加其他 mount 直接改 `docker-compose.yml`。
